...
class CryptoTransaction(models.Model):
    ...
    def process(self):
        ...
        transactions = self.filtered(
            lambda x: x.bank_account_id.bank_id.crypto_provider == "okx"
        )
        if not transactions:
            return

        currencies = {
            cur.okx_api_code: cur
            for cur in self.env["res.currency"].search([("okx_api_code", "!=", False)])
        }
        ...
        for transaction in transactions:
            try:
                for tx in transaction.input_ids:
                    ...
                    if tx.from_csv:
                        d = datetime.strptime(
                            data["create_time"], "%Y/%m/%d %H:%M"
                        )  # "%d.%m.%Y %H:%M:%S"
                        if data["biz_line_en"] == "spot":
                            trade_name = "Trade OKX ({}, {})".format(
                                d, data["instrument_name"]
                            )
                            self.env["crypto.transaction.detail"].create({
                                ...
                                "value":
                                    float(data["amount"]) *
                                    (-1 if data["bill_type_en"] == "sell" else 1),
                            })
