...
class CryptoTransactionDetail(models.Model):
    ...
    def _generate_statement_lines(self, group_by=None):
        journals = {(j.bank_account_id.id, j.currency_id.id): j
        ...
        for tx in self:
            ...
            if skey not in statements:
                ...
                statements[skey] = {
                    ...
                    "name":
                        "{} - {}".format(
                            journal.bank_account_id.bank_name or
    ...
    def _slit_statements(self, statements):
        LINES_LIMIT = int(
            self.env["ir.config_parameter"].sudo().get_param(
                "crypto_sync.statement_lines_limit", 0
            )
        )
        ...
        for statement in statements:
            ...
            statement["line_ids"] = sorted(
                statement["line_ids"], key=lambda x: x[2]["date"]
            )

            for i in range(statement["_total"]):
                new_statement = statement.copy()
                new_statement["_part"] = i + 1
                new_statement["line_ids"] = statement["line_ids"][i *
                                                                  LINES_LIMIT:(i + 1) *
                                                                  LINES_LIMIT]
    ...
    def _clean_statements(self, statements, group_by=None):
        for statement in statements:
            first = min([line[2] for line in statement["line_ids"]],
                        key=lambda l: l["date"])
            last = max([line[2] for line in statement["line_ids"]],
                       key=lambda l: l["date"])
            if not group_by:
                ...
                statement["date"] = date(
                    statement["_period"][1] +
                    (1 if statement["_period"][0] == 12 else 0),
